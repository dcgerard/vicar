...
This vignette describes how to build your own factor analysis functions for use in vicar's primary functions `vruv4`, `ruv3`, `mouthwash`, `backwash`, and `ruvb`.
...
The default value for `fa_func` is `bfa_gs_linked`. We can test a factor anaysis for compatibility with `ruvb` using the function `fa_tester_ruvb`.
```{r}
